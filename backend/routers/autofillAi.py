...
def _user_prompt(
    ...
    department_from_image: Optional[str],
    ...
    hints = {
        ...
        "address_hint": address or "not available",
    }
    return (
        ...
        "If an 'address_hint' is provided, integrate it naturally into the description."
    )
...
async def ai_assist(
    ...
    address: Optional[str] = Form(default=None), # <-- Added from friend's version
    image: Optional[UploadFile] = File(default=None), # <-- Kept from your version
):
    ...
    # We now pass the address to the merged _user_prompt function
    messages = [
        {"role": "system", "content": _system_prompt()},
        {"role": "user", "content": _user_prompt(title, description, predicted_department, address)},
    ]
